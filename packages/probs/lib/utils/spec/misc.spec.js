...
test('queueManager works', async () => {
    ...
    assert(
        duration < 190 && duration >= 150,
        `duration should be < 190 and >= 150. It was ${duration}`,
    )
})
...
test('queueManager works in single sequence', async () => {
    ...
    // we expect around 150 because the test lasts 50 ms and the first free slot is available at 100 ms
    assert(
        duration < 400 && duration >= 350,
        `duration should be < 400 and >= 350. It was ${duration}`,
    )
})

test('importCfg', () => {
    test('should fail if config has bad format', async () => {
        let err
        try {
            await importCfg(__dirname + '/_badCfgSample.js')
        } catch (_err) {
            err = _err
        }
        assert.equal(err.name, 'ParseError')
    })
    test('should fail if require is true and config is missing', async () => {
        let err
        try {
            await importCfg(__dirname + '/_missingCfg.js', true)
        } catch (_err) {
            err = _err
        }
