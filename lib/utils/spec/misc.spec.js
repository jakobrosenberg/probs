...
test('createQueuedFunctionWrapper functions run in correct order atCreate', async () => {
    ...
    const q1 = qfWrapper(() => results.push(1))
    const q2 = qfWrapper(() => results.push(2))
    const q3 = qfWrapper(() => results.push(3))
    const q4 = qfWrapper(() => results.push(4))

    assert.equal(qfWrapper.queue.length, 4)
    
    // execute functions in random order
    const promises = [q2(), q4(), q1(), q3()]
