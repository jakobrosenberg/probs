...
export class FilesQueue {
    ...
    async add(fileItem) {
        ...
        this.options.reporter.addedFile({ scope: [file], fileItem })

        this.pending.push(fileItem)
        if (!this.options.workers) this.#processQeueWithoutWorker(fileItem)
        if (this.running.length < this.options.workers)
