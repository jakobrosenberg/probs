...
export const _transform = async (content, file = 'na') => {
    ...
    tests.forEach(test => {
        ...
        const insert = [
            ...
            `try { test = PROBS_CONTEXT.nestedTest } catch {}`,
            `try { expect.toMatchSnapshot = PROBS_GLOBAL_CONTEXT.createSnapshotMatcher(PROBS_CONTEXT) } catch {}`,
            '/* start of compiled probs code */',
            '',
        ].join('\r\n' + padding)
...
export async function transformSource(source, opts) {
    if (opts.url.endsWith(process.env.__probsTestFile) || opts.url.match(/\.test\.js/)) {
