...
import { createHooksCollection, createQueuedFunctionWrapper, resolveConfig } from './utils/misc.js'
...
export const processFile = async (file, options = {}) => {
    ...
    if (options.globals) {
        const test = createTest([file], { options, hooks, createQueuedFunction }, promiseQueue)
        const globals = {
            test,
            it: test,
            describe: test,
