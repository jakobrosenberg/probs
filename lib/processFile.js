 ...
 * @param {string} file
 * @param {(eventName: ProbEvents, params: {}) => void} emitter
 * @param {any} options
 */
export const processFile = async (file, emitter, options = {}) => {
    ...
    const promiseQueue = []
    const createQueuedFunction = createQueuedFunctionWrapper('atCreate')
    ...
    if (options.globals) {
        const test = createTest({
            ...
            emitter,
            parentPromiseQueue: promiseQueue,
        })
        const globals = {
            test,
            it: test,
